...
from networks import network_mutilstage
...
def main(args):
    ...
    # create model
    model = network_mutilstage.__dict__[cfg.model](cfg.output_shape, cfg.num_class, pretrained=True)
...
def train(train_loader, model, criterions, optimizer, epoch, clr, write):
    ...
    for i, (inputs, targets, valid, meta) in enumerate(train_loader):
        ...
        # compute output
        global_output1, global_output2, refine_output1, refine_output2 = model(input_var)
        # global_x4, global_x3, global_x2, global_output = global_outputs
        score_map = refine_output2.data.cpu()

        loss = 0.
        global_loss_record = 0.
        refine_loss_record = 0.
        # comput global loss and refine loss
        num_points = global_output1.size()[1]
        global_label = target7 * (valid > 0.1).type(torch.FloatTensor).view(-1, num_points, 1, 1)
        global_loss = criterion1(global_output1, torch.autograd.Variable(global_label.cuda(async=True)))
        loss += global_loss
        global_loss_record += global_loss.data.item()

        global_label = target7 * (valid > 0.1).type(torch.FloatTensor).view(-1, num_points, 1, 1)
        global_loss = criterion1(global_output2, torch.autograd.Variable(global_label.cuda(async=True)))
        loss += global_loss
        global_loss_record += global_loss.data.item()
        ...
        refine_loss = criterion2(refine_output1, refine_target_var)
        ...
        refine_loss = criterion2(refine_output2, refine_target_var)
        refine_loss = refine_loss.mean(dim=3).mean(dim=2)
        refine_loss *= (valid_var > -0.9).type(torch.cuda.FloatTensor)
        refine_loss = ohkm(refine_loss, 8)
        loss += refine_loss
